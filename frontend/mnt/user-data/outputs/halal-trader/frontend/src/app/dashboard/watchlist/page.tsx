...
export default function WatchlistPage() {
  ...
  const loadWatchlist = async () => {
    ...
    setItems(data);
    // Load halal screen for each
    const screenResults: Record<string, HalalScreenResult> = {};
    await Promise.all(
      data.map(async (item) => {
        try {
          screenResults[item.symbol] = await api.halal.screen(item.symbol);
        } catch {}
      })
    );
    setScreens(screenResults);
  };
  ...
  const statusBadge = (status: string) => {
    const map: Record<string, string> = {
      ...
      doubtful: "badge-amber", unknown: "badge-muted",
    };
  ...
  return (
    <div>
      ...
      <div className="card" style={{ marginBottom: 24 }}>
        ...
        <p style={{ fontSize: 11, color: "var(--text3)", fontFamily: "var(--mono)", marginTop: 8 }}>
          Stocks are screened via Zoya API before being added. Non-compliant stocks are rejected.
        </p>
      </div>
      ...
      <div className="card">
        ...
        {loading ? (
          ...
          <p style={{ color: "var(--text3)", fontFamily: "var(--mono)", fontSize: 12 }}>
            No stocks on watchlist yet. Add one above.
          </p>
        ) : (
                  ...
                  <tr key={item.id}>
                    ...
                    <td>
                      <span style={{ fontFamily: "var(--mono)", fontSize: 12, color: "var(--text2)" }}>
                        {s?.debt_ratio != null ? `${(s.debt_ratio * 100).toFixed(1)}%` : "—"}
                      </span>
                    </td>
                    <td>
