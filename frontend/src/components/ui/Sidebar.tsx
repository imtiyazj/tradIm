    ...
    <aside className="sidebar">
      ...
      {NAV.map((item) => (
        <Link key={item.href} href={item.href} className={`nav-item ${pathname === item.href ? "active" : ""}`}>
          <span style={{ fontFamily:"var(--mono)", fontSize:"14px" }}>{item.icon}</span>
          {item.label}
        </Link>
      ))}
      <div style={{ flex:1 }} />
      <div style={{ padding:"16px 20px", borderTop:"1px solid var(--border)", display:"flex", alignItems:"center", gap:"10px" }}>
        <UserButton appearance={{ variables:{ colorPrimary:"#00c896" }, elements:{ avatarBox:{ width:28, height:28 } } }} />
        <span style={{ fontSize:"12px", color:"var(--text2)", fontFamily:"var(--mono)" }}>Account</span>
      </div>
    </aside>
  );
}
