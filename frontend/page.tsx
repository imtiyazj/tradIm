...
function SignalBadge({ type }: { type: string }) {
  const map: Record<string, string> = {
    ...
    watch: "badge-amber", avoid: "badge-muted",
  };
  return <span className={`badge ${map[type] || "badge-muted"}`}>{type}</span>;
}

function ConfBar({ value }: { value: number }) {
  const color = value >= 0.75 ? "var(--green)" : value >= 0.6 ? "var(--amber)" : "var(--text3)";
  return (
...
export default function DashboardPage() {
  ...
  const triggerJob = async () => {
    setRunning(true);
    try {
      await api.scheduler.run();
      showToast("Morning job triggered. Check Telegram for updates.");
    } catch (e: unknown) {
  ...
  const todaySignals = signals.filter(s => {
    const d = new Date(s.triggered_at);
    const now = new Date();
    ...
    <div>
      ...
      <div className="grid-4" style={{ marginBottom: 24 }}>
        <StatCard
          ...
          sub={`${portfolio?.count || 0} positions`}
          color={totalPnL >= 0 ? "var(--green)" : "var(--red)"}
        />
        <StatCard
          label="Today's signals"
          value={todaySignals.length}
          sub={`${todaySignals.filter(s => s.type === "buy").length} buy · ${todaySignals.filter(s => s.type === "watch").length} watch`}
        />
        <StatCard
          label="Screened today"
          value={report?.stocks_screened || 0}
          sub={`${report?.halal_passed || 0} halal passed`}
        />
        <StatCard
          label="Alerts sent"
          value={report?.alerts_sent || 0}
          sub={report?.report_date || "No report yet"}
        />
      </div>

      {/* Layout: signals + report */}
      <div className="grid-2">
        {/* Latest signals */}
        <div className="card">
          <div className="card-header">
            <span className="card-title">Latest signals</span>
            <a href="/dashboard/signals" style={{ fontSize: 11, color: "var(--text3)", fontFamily: "var(--mono)", textDecoration: "none" }}>
              View all →
            </a>
          </div>
          {signals.length === 0 ? (
            <p style={{ color: "var(--text3)", fontSize: 12, fontFamily: "var(--mono)" }}>
              No signals yet. Run the morning job to generate signals.
            </p>
          ) : signals.map(s => (
            <div key={s.id} className={`signal-card ${s.type}`}>
              ...
              {s.confidence && <ConfBar value={s.confidence} />}
        ...
        <div>
          <div className="card" style={{ marginBottom: 16 }}>
            <div className="card-header">
              ...
              <a href="/dashboard/reports" style={{ fontSize: 11, color: "var(--text3)", fontFamily: "var(--mono)", textDecoration: "none" }}>
                History →
              </a>
            </div>
            {report ? (
              <>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                  {[
                    ["Screened", report.stocks_screened],
                    ["Halal passed", report.halal_passed],
                    ["Signals fired", report.signals_fired],
              ...
              <p style={{ color: "var(--text3)", fontSize: 12, fontFamily: "var(--mono)" }}>
                No report yet. Trigger the morning job to generate one.
              </p>
            )}
          </div>
          ...
          <div className="card">
            <div className="card-header">
              ...
              <span className={`badge ${mode === "paper" ? "badge-amber" : "badge-green"}`}>
                {mode}
              </span>
            </div>
